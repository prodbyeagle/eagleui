...
export const Slider: React.FC<SliderProps> = ({
	...
	return (
		<div className={`w-full ${className}`}>
			...
			<div
				ref={sliderRef}
				className={`relative h-12 select-none ${disabled ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''}`}
			>
				<span className='absolute top-1/2 left-0 right-0 h-1 -translate-y-1/2 bg-neutral-200 dark:bg-neutral-700 rounded-full' />

				<div
					className='absolute top-1/2 left-0 h-1 -translate-y-1/2 bg-neutral-900 dark:bg-neutral-200 rounded-full'
				...
				{getMarkerPositions().map((markerValue) => (
					<div
						...
						} ${variant === 'sticky' ? 'hover:scale-150 transition-transform' : ''}`}
						style={{
				...
				<div
					className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 bg-white dark:bg-neutral-900 border-2 border-neutral-900 dark:border-neutral-200 rounded-full cursor-pointer shadow-sm hover:scale-110 transition-transform ${
				...
				>
					<span
						className={cn(`absolute bottom-full mb-2 px-2 py-1 pointer-events-none text-xs border border-neutral-600/50 font-medium rounded-lg ${variant !== 'none' ? 'bg-neutral-950 text-neutral-100' : 'hidden'} transition duration-300 ${isDragging ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`)}
